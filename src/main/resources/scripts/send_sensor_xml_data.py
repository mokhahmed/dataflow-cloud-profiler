...
def publish(publisher, topic, events):
   ...
   numobs = len(events)
   sensor_xml_events = [sensor_str_to_xml(str(ev)).encode() for ev in events]
   if numobs > 0:
       logging.info('Publishing {0} events from {1}'.format(numobs, get_timestamp(events[0])))
       for event_data in sensor_xml_events:
...
if __name__ == '__main__':
   ## python3 send_sensor_xml_data.py --project moka-playground --speedFactor 60
   ...
   topic_name = 'projects/{project_id}/topics/{topic}'.format(project_id=args.project,topic=TOPIC)

   #event_type = publisher.topic_path(args.project,TOPIC)
   #try:
   #publisher.get_topic(name= topic_name)
   ...
   with gzip.open(INPUT, 'rb') as ifp:
      ...
      simulate(topic_name, ifp, firstObsTime, programStartTime, args.speedFactor)
